...
- `renovate.json` contains `prBodyColumns` with `"Source"` and `prBodyDefinitions.Source` template.
- `gh` CLI is authenticated.

## Step 0 — Collect project runtime requirements

Before looking at any PR, harvest the project's pinned runtime versions (JDK, Node, Gradle wrapper, etc.). These values become the judgement baseline in Step 7. The skill must work in any repository, so do NOT hardcode values like "JDK 21". Instead, walk the source list below in order and stop when a concrete version is found for each runtime. Record every hit with `file:line` so the subagents can cite it in the report.
...
1. `gradle/libs.versions.toml` (TOML, primary SSOT for Gradle projects)
2. `gradle/wrapper/gradle-wrapper.properties` (`distributionUrl` pins the Gradle wrapper)
3. `build.gradle.kts` / `build.gradle` — look for `jvmToolchain(N)` or `JavaLanguageVersion.of(N)`
4. `.github/workflows/*.yml` — `java-version: 'N'` or the input to `setup-java@`
5. `Dockerfile` — the `FROM image:tag` line on any final runtime stage
6. `.tool-versions` / `.nvmrc` / `.java-version`
7. `package.json` — the `engines` block
8. `CLAUDE.md` (project root and `.claude/CLAUDE.md`)
9. `README.md` / `README.*` — technology-stack, prerequisites, or requirements sections
...
|---------|---------|--------|
| java    | 21      | build.gradle.kts:18 / Dockerfile:1 |
| gradle  | 9.3.0   | gradle/wrapper/gradle-wrapper.properties:3 |
```
...
bash .claude/skills/check-update-all-dependencies/scripts/apply-gotchas.sh \
  gradle/libs.versions.toml renovate.json < facts.json > warnings.json
```
...
- `prBodyDefinitions.Source` must exist in `renovate.json`. Without it, Source URL falls back to Maven Central and network failures cascade.
